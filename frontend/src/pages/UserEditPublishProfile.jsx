...
import "./UserEditPublishProfile.css";
...
export default function UserEditPublishProfile() {
  ...
  const [profile, setProfile] = useState({
    ...
    bgColor: "#f0f4f8",
    bgPattern: "none",
    cardType: "business", // 'business' or 'shopkeeper'
  ...
  const [previewMode, setPreviewMode] = useState("mobile"); // 'mobile' or 'web'
  ...
  // Validation: Check if the last item of any list is filled
  ...
  const addCustomItem = (headingId) => {
    setCustomHeadings(customHeadings.map(h => {
      if (h.id === headingId) {
        if (canAddMore(h.items, ["title", "subtitle"])) {
  ...
  return (
    <div className="profile-editor-container">
      <header className="editor-nav">
        <span className="nav-brand" onClick={() => navigate("/UserDashboard")} style={{ cursor: "pointer" }}>Prism QR</span>
      ...
      <main className="editor-main">
        <section className="editor-content">
          ...
          {sectionOrder.map((sectionKey, orderIndex) => {
            ...
            // Hide specific sections based on card type
                ...
                <div key="products" className="editor-section">
                   <div className="section-header">
                     <div className="section-title">
                       ...
                       <h2>Product Catalog ({products.length}/50)</h2>
                     </div>
                        ...
                        <div key={prod.id} className="dynamic-item complex">
                           <div className="item-row">
                             <div className="move-controls">
                               <button className="material-symbols-outlined" onClick={() => moveItem(products, setProducts, pIdx, "up")}>expand_less</button>
                             </div>
                             <div className="icon-upload-mini">
                               <input type="file" accept="image/*" onChange={(e) => {
                                  ...
                                  if (file) {
                                    ...
                                    reader.onloadend = () => setProducts(products.map(p => p.id === prod.id ? { ...p, icon: reader.result } : p));
                           ...
                           <div className="item-row flex-between mt-2" style={{ padding: '0.5rem 0.75rem', background: 'var(--surface-container-low)', borderRadius: '8px' }}>
                             <div className="flex-center gap-2">
                               <label className="switch mini">
                                 <input type="checkbox" checked={prod.isOutOfStock} onChange={() => setProducts(products.map(p => p.id === prod.id ? { ...p, isOutOfStock: !p.isOutOfStock } : p))} />
                             ...
                             <div className="flex-center gap-2">
                                <label className="switch mini">
                                  <input type="checkbox" checked={prod.isActive} onChange={() => toggleActive(products, setProducts, prod.id)} />
                                  <span className="slider round"></span>
                                </label>
                ...
                <div key="links" className="editor-section">
                  <div className="section-header">
                    <div className="section-title">
                      ...
                      <h2>Social links & Icons ({dynamicSections.length}/8)</h2>
                    </div>
                    ...
                    {dynamicSections.map((section, index) => (
                      <div key={section.id} className="dynamic-item">
                        ...
                        <label className="switch">
                          <input type="checkbox" checked={section.isActive} onChange={() => toggleActive(dynamicSections, setDynamicSections, section.id)} />
                          <span className="slider round"></span>
                        </label>
                ...
                <div key="projects" className="editor-section">
                  <div className="section-header">
                    <div className="section-title">
                      ...
                      <h2>Projects Portfolio ({projects.length}/50)</h2>
                    </div>
                    ...
                    {projects.map((proj, index) => (
                      <div key={proj.id} className="dynamic-item complex">
                        ...
                        <div className="item-row flex-between">
                          ...
                          <label className="switch mini">
                            <input type="checkbox" checked={proj.isActive} onChange={() => toggleActive(projects, setProjects, proj.id)} />
                            <span className="slider round"></span>
                          </label>
                      ...
                      <div key={h.id} className="dynamic-item-wrapper custom-heading-container">
                        <div className="heading-main-row">
                          ...
                          <div className="flex-center gap-2">
                            ...
                            <label className="switch mini">
                              <input type="checkbox" checked={h.isActive} onChange={() => toggleActive(customHeadings, setCustomHeadings, h.id)} />
                              <span className="slider round"></span>
                            </label>
                            ...
                            <div key={item.id} className="dynamic-item mini">
                              <div className="move-controls">
                                <button className="material-symbols-outlined" onClick={() => {
                                  const newHeadings = [...customHeadings];
                                  const newItems = [...newHeadings[hIndex].items];
                                  const newI = iIndex - 1;
                                  if(newI >= 0) {
                                    [newItems[iIndex], newItems[newI]] = [newItems[newI], newItems[iIndex]];
                                    newHeadings[hIndex].items = newItems;
                                    setCustomHeadings(newHeadings);
                                  }
                                }}>expand_less</button>
                              </div>
              ...
              <div className="phone-mockup">
                <div className="phone-screen-scroll-area no-scrollbar" style={{ background: profile.bgColor }}>
                  <div className="phone-screen-fixed" style={{ padding: '3rem 2rem 1.5rem', borderBottom: `1px solid ${profile.subTextColor}20` }}>
                    ...
                    {/* Contact Info Preview (Mobile) */}
                    {(profile.showMobile || profile.showEmail || profile.showAddress) && (
                      <div className="contact-info-preview-grid" style={{ padding: '0 2rem' }}>
                         {profile.showMobile && profile.mobile && (
                           <div className="contact-preview-item" style={{ color: profile.subTextColor }}>
                             <span className="material-symbols-outlined" style={{ fontSize: '1rem', color: profile.themeColor }}>call</span>
                             {profile.mobile}
                           </div>
                         )}
                         {profile.showEmail && profile.email && (
                           <div className="contact-preview-item" style={{ color: profile.subTextColor }}>
                             <span className="material-symbols-outlined" style={{ fontSize: '1rem', color: profile.themeColor }}>mail</span>
                             {profile.email}
                           </div>
                         )}
                         {profile.showAddress && profile.address && (
                           <div className="contact-preview-item" style={{ color: profile.subTextColor }}>
                             <span className="material-symbols-outlined" style={{ fontSize: '1rem', color: profile.themeColor }}>location_on</span>
                             <span style={{ fontSize: '0.75rem', lineHeight: 1.3 }}>{profile.address}</span>
                           </div>
                         )}
                      </div>
                    )}
                  ...
                  <div className="preview-content-sections" style={{ padding: '0 2rem 4rem' }}>
                    {sectionOrder.map(sectionKey => {
                      // Skip sections based on card type
                      ...
                      if (sectionKey === "links" && dynamicSections.some(s => s.isActive)) {
                        return (
                          <div key="links" className="preview-links-grid">
                            {dynamicSections.filter(s => s.isActive).map(s => (
                              <div key={s.id} className="preview-link-circle" style={{ color: profile.themeColor, background: `${profile.themeColor}15` }}>
                                {s.icon ? <img src={s.icon} className="custom-icon" /> : <span className="material-symbols-outlined">link</span>}
                              </div>
                            ))}
                          </div>
                        );
                      ...
                      if (sectionKey === "products" && products.some(p => p.isActive)) {
                        return (
                          <div key="products" className="preview-list-section">
                            <h5 style={{ color: profile.themeColor }}>Product Catalog</h5>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                              {products.filter(p => p.isActive).map(p => (
                                <div key={p.id} className="mini-card-preview" style={{ background: `${profile.textColor}08`, display: 'flex', gap: '0.75rem', opacity: p.isOutOfStock ? 0.6 : 1 }}>
                                  <div style={{ width: '50px', height: '50px', background: 'white', borderRadius: '8px', overflow: 'hidden', flexShrink: 0 }}>
                                    {p.icon ? <img src={p.icon} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span className="material-symbols-outlined" style={{ fontSize: '1.5rem', color: '#ccc', display: 'flex', justifyContent: 'center', height: '100%', alignItems: 'center' }}>image</span>}
                                  </div>
                                  <div style={{ flex: 1 }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                      <p className="p-bold" style={{ color: profile.textColor }}>{p.title}</p>
                                      <span style={{ fontSize: '0.7rem', fontWeight: 800, color: profile.themeColor }}>₹{p.price}</span>
                                    </div>
                                    <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.25rem' }}>
                                      {p.isOutOfStock ? (
                                        <span className="product-tag out">Out of Stock</span>
                                      ) : (
                                        <span className="product-tag stock">{p.stock} Available</span>
                                      )}
                                      {p.offer && <span className="product-tag offer">{p.offer}% Off</span>}
                                    </div>
                                  </div>
                                </div>
                              ))}
                      ...
                      if (sectionKey === "projects" && products.some(p => p.isActive)) {
                        return (
                          <div key="projects" className="preview-list-section">
                            <h5 style={{ color: profile.themeColor }}>Projects Portfolio</h5>
                            {projects.filter(p => p.isActive).map(p => (
                              <div key={p.id} className="mini-card-preview" style={{ background: `${profile.textColor}08` }}>
                                <p className="p-bold" style={{ color: profile.textColor }}>{p.title}</p>
                      ...
                      if (sectionKey === "experience" && experiences.some(e => e.isActive)) {
                        return (
                          <div key="experience" className="preview-list-section">
                            <h5 style={{ color: profile.themeColor }}>Professional Exp.</h5>
                            {experiences.filter(e => e.isActive).map(e => (
                              <div key={e.id} className="mini-card-preview" style={{ background: `${profile.textColor}08` }}>
                          ...
                          <div key="custom">
                            {customHeadings.filter(h => h.isActive).map(h => (
                               <div key={h.id} className="preview-list-section">
                                  <h5 style={{ color: profile.themeColor }}>{h.title}</h5>
                                  {h.items.filter(it => it.isActive).map(it => (
                                    <div key={it.id} className="mini-card-preview" style={{ background: `${profile.textColor}08` }}>
                      ...
                      if (sectionKey === "daily" && dailyActivities.some(a => a.isActive)) {
                        return (
                          <div key="daily" className="preview-list-section">
                            <h5 style={{ color: profile.themeColor }}>Routine</h5>
                            {dailyActivities.filter(a => a.isActive).map(a => (
                              <div key={a.id} className="mini-card-preview" style={{ background: `${profile.textColor}08`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <p className="p-bold" style={{ color: profile.textColor }}>{a.title}</p>
                                <span style={{ fontSize: '10px', color: profile.themeColor, fontWeight: 800 }}>{a.time}</span>
                              </div>
                            ))}
                          </div>
                        );
                      ...
                      if (sectionKey === "languages" && languages.some(l => l.isActive)) {
                        return (
                          <div key="languages" className="preview-list-section">
                            <h5 style={{ color: profile.themeColor }}>Languages</h5>
                            <div className="tags-preview">
                              {languages.filter(l => l.isActive).map(l => <span key={l.id} style={{ color: profile.textColor, background: `${profile.themeColor}15` }}>{l.name}</span>)}
                            </div>
                          </div>
                        );
                      }
                      ...
                      if (sectionKey === "hobbies" && hobbies.some(h => h.isActive)) {
                        return (
                          <div key="hobbies" className="preview-list-section">
                            <h5 style={{ color: profile.themeColor }}>Hobbies</h5>
                            <div className="tags-preview">
                              {hobbies.filter(h => h.isActive).map(h => <span key={h.id} style={{ color: profile.textColor, background: `${profile.themeColor}15` }}>{h.name}</span>)}
                            </div>
                          </div>
                        );
                      }
                ...
                <div className="web-preview-container no-scrollbar" style={{ background: profile.bgColor }}>
                  <div className="web-preview-content" style={{ background: 'white' }}>
                    <div className="web-profile-intro" style={{ textAlign: 'center', marginBottom: '3rem' }}>
                      ...
                      {/* Contact Info Preview (Web) */}
                      {(profile.showMobile || profile.showEmail || profile.showAddress) && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '2rem', marginTop: '1.5rem' }}>
                           {profile.showMobile && profile.mobile && (
                             <div className="contact-preview-item" style={{ color: profile.subTextColor, fontSize: '0.9rem' }}>
                               <span className="material-symbols-outlined" style={{ color: profile.themeColor, fontSize: '1.25rem' }}>call</span>
                               {profile.mobile}
                             </div>
                           )}
                           {profile.showEmail && profile.email && (
                             <div className="contact-preview-item" style={{ color: profile.subTextColor, fontSize: '0.9rem' }}>
                               <span className="material-symbols-outlined" style={{ color: profile.themeColor, fontSize: '1.25rem' }}>mail</span>
                               {profile.email}
                             </div>
                           )}
                           {profile.showAddress && profile.address && (
                             <div className="contact-preview-item" style={{ color: profile.subTextColor, fontSize: '0.9rem' }}>
                               <span className="material-symbols-outlined" style={{ color: profile.themeColor, fontSize: '1.25rem' }}>location_on</span>
                               {profile.address}
                             </div>
                           )}
                        </div>
                      ...
                      <div className="web-grid-sections" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '3rem', marginTop: '3rem', textAlign: 'left' }}>
                        {sectionOrder.map(sectionKey => {
                           // Skip sections based on card type
                           if (profile.cardType === 'shopkeeper' && ["projects", "experience", "hobbies"].includes(sectionKey)) return null;
                           if (profile.cardType === 'business' && sectionKey === "products") return null;
                          ...
                          if (sectionKey === "links" && dynamicSections.some(s => s.isActive)) {
                            return (
                              <div key="links" style={{ gridColumn: '1 / -1' }}>
                                <div className="preview-links-grid" style={{ justifyContent: 'center', marginBottom: '2rem' }}>
                                  {dynamicSections.filter(s => s.isActive).map(s => (
                                    <div key={s.id} className="preview-link-circle" style={{ width: '64px', height: '64px', color: profile.themeColor, background: `${profile.themeColor}15` }}>
                                      {s.icon ? <img src={s.icon} className="custom-icon" /> : <span className="material-symbols-outlined" style={{ fontSize: '1.75rem' }}>link</span>}
                                    </div>
                                  ))}
                                </div>
                              </div>
                          ...
                          if (sectionKey === "products" && products.some(p => p.isActive)) {
                            return (
                              <div key="products" style={{ gridColumn: '1 / -1' }}>
                                <h5 style={{ color: profile.themeColor, fontSize: '1.25rem', marginBottom: '2rem' }}>Product Catalog</h5>
                                   ...
                                   {products.filter(p => p.isActive).map(p => (
                                     <div key={p.id} className="web-product-card" style={{ opacity: p.isOutOfStock ? 0.7 : 1 }}>
                                        <div className="web-product-img">
                                           {p.icon ? <img src={p.icon} /> : <span className="material-symbols-outlined" style={{ fontSize: '3rem', color: '#eee' }}>image</span>}
                                           {p.isOutOfStock && (
                                             <div style={{ position: 'absolute', inset: 0, background: 'rgba(255,255,255,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: 'var(--error)' }}>
                                                SOLD OUT
                                             </div>
                                           )}
                                        </div>
                                        <div className="web-product-info">
                                           <p className="p-bold" style={{ fontSize: '1rem', color: profile.textColor }}>{p.title}</p>
                                           ...
                                           <div style={{ marginTop: '0.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                              <span style={{ fontSize: '0.75rem', color: profile.subTextColor }}>Stock: {p.stock}</span>
                                              {!p.isOutOfStock && <button style={{ background: profile.themeColor, color: 'white', border: 'none', padding: '4px 12px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 700 }}>Inquire</button>}
                                           </div>
                                        </div>
                                     </div>
                                   ))}
                          ...
                          if (sectionKey === "projects" && projects.some(p => p.isActive)) {
                            return (
                              <div key="projects" className="preview-list-section">
                                <h5 style={{ color: profile.themeColor, fontSize: '1.25rem' }}>Projects Portfolio</h5>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                  {projects.filter(p => p.isActive).map(p => (
                                    <div key={p.id} className="mini-card-preview" style={{ margin: 0, padding: '1.5rem', background: `${profile.textColor}05` }}>
                                      <p className="p-bold" style={{ fontSize: '1.1rem', color: profile.textColor }}>{p.title}</p>
                                      <p className="p-sub" style={{ margin: '0.4rem 0', color: profile.subTextColor }}>{p.type}</p>
                                      <p style={{ fontSize: '0.875rem', color: profile.subTextColor, lineHeight: 1.5 }}>{p.description}</p>
                                    </div>
                                  ))}
                                </div>
                              </div>
                          ...
                          if (sectionKey === "experience" && experiences.some(e => e.isActive)) {
                            return (
                              <div key="experience" className="preview-list-section">
                                <h5 style={{ color: profile.themeColor, fontSize: '1.25rem' }}>Professional Experience</h5>
                                {experiences.filter(e => e.isActive).map(e => (
                                  <div key={e.id} className="mini-card-preview" style={{ padding: '1.5rem', background: `${profile.textColor}05` }}>
                                    <p className="p-bold" style={{ fontSize: '1.1rem', color: profile.textColor }}>{e.role}</p>
                                    <p className="p-sub" style={{ color: profile.subTextColor }}>{e.company} • {e.start} - {e.isCurrent ? "Present" : e.end}</p>
                                  </div>
                                ))}
                              </div>
                          ...
                          if (sectionKey === "custom" && customHeadings.some(h => h.isActive)) {
                            return (
                              <div key="custom">
                                {customHeadings.filter(h => h.isActive).map(h => (
                                  <div key={h.id} className="preview-list-section">
                                    <h5 style={{ color: profile.themeColor, fontSize: '1.25rem' }}>{h.title}</h5>
                                    {h.items.filter(it => it.isActive).map(it => (
                                      <div key={it.id} className="mini-card-preview" style={{ padding: '1.25rem 1.5rem', background: `${profile.textColor}05` }}>
                                        <p className="p-bold" style={{ fontSize: '1.05rem', color: profile.textColor }}>{it.title}</p>
                                        <p className="p-sub" style={{ color: profile.subTextColor }}>{it.subtitle}</p>
                                      </div>
                                    ))}
                                  </div>
                          ...
                          if (sectionKey === "daily" && dailyActivities.some(a => a.isActive)) {
                            return (
                              <div key="daily" className="preview-list-section">
                                <h5 style={{ color: profile.themeColor, fontSize: '1.25rem' }}>Daily Routine</h5>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                  {dailyActivities.filter(a => a.isActive).map(a => (
                                    <div key={a.id} className="mini-card-preview" style={{ margin: 0, padding: '1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: `${profile.textColor}05` }}>
                                      <p className="p-bold" style={{ color: profile.textColor }}>{a.title}</p>
                                      <span style={{ fontSize: '0.8rem', padding: '4px 10px', background: `${profile.themeColor}15`, color: profile.themeColor, borderRadius: '6px', fontWeight: 800 }}>{a.time}</span>
                                    </div>
                                  ))}
                                </div>
                              </div>
                          ...
                          if (sectionKey === "languages" && languages.some(l => l.isActive)) {
                            return (
                              <div key="languages" className="preview-list-section">
                                <h5 style={{ color: profile.themeColor, fontSize: '1.25rem' }}>Languages</h5>
                                <div className="tags-preview" style={{ flexWrap: 'wrap', gap: '1rem' }}>
                                  {languages.filter(l => l.isActive).map(l => <span key={l.id} style={{ padding: '10px 20px', fontSize: '0.95rem', color: profile.textColor, background: `${profile.themeColor}10` }}>{l.name}</span>)}
                                </div>
                              </div>
                            );
                          }
                          ...
                          if (sectionKey === "hobbies" && hobbies.some(h => h.isActive)) {
                            return (
                              <div key="hobbies" className="preview-list-section">
                                <h5 style={{ color: profile.themeColor, fontSize: '1.25rem' }}>Interests & Hobbies</h5>
                                <div className="tags-preview" style={{ flexWrap: 'wrap', gap: '1rem' }}>
                                  {hobbies.filter(h => h.isActive).map(h => <span key={h.id} style={{ padding: '10px 20px', fontSize: '0.95rem', color: profile.textColor, background: `${profile.themeColor}10` }}>{h.name}</span>)}
                                </div>
                              </div>
                            );
                          }
